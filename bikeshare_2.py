...
def load_data(city, month, day):
    ...
    # loading a data file into a dataframe
    df = pd.read_csv(CITY_DATA[city])

    # converting Start Time to datetime
    df['Start Time'] = pd.to_datetime(df['Start Time'])

    #  # extract month and day from Start Time to make new columns
    df['month'] = df['Start Time'].dt.month
    df['day_of_week'] = df['Start Time'].dt.weekday_name

    # filtering by month
    if month != 'all':
        # indexing  by months to get the corresponding int
        months = ['january', 'february', 'march', 'april', 'may', 'june']
        month = months.index(month) + 1

        # filtering by month to make a dataframe
        df = df[df['month'] == month]

    # filter by day of the week;
    if day != 'all':
        # filtering by day to make a new dataframe
...
def user_stats(df):
    ...
    if city != 'washington':
        ...
        mryob = sorted(df.groupby(['Birth Year'])[
